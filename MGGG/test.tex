...
\chno{1}{Test Document}{Professor Q Fictional}{Zach Schutzman}
...
\defn{An \textbf{alphabet} is a non-empty, finite set of characters.}
\defn{A \textbf{string} $s$ (over an alphabet $\Sigma$) is a finite ordered sequence of elements of $\Sigma$.}
\defn{The \textbf{empty string}, $\epsilon$, is the sequence of no symbols, and is in fact a valid string.}
\defn{Let \textbf{$\Sigma^*$} be the set of all strings over $\Sigma$.}
\defn{A \textbf{language} over $\Sigma$ is any subset of $\Sigma^*$.}
...
\defn{A \textbf{finite state machine} will be a model with a constant amount of memory.}
...
\defn{A \textbf{deterministic finite automaton (DFA), $M$, is a 5-tuple $(Q,\Sigma , \delta , q_0, F)$.}}
...
\defn{$M$ \textbf{accepts} a string $s=s_1 s_2 \dots s_k$ if there is a sequence of states in $M$ starting with $q_0$ and ending in a final state $q_0 q_1 \dots q_k$ such that $\delta(q_i,s_{i+1}) = q_{i+1}$.}

\defn{If $\mathcal{L} = \{s | M \ accepts \ s\}$, the we say $M$ \textbf{recognizes} $\mathcal{L}$.}
\defn{If a language $\mathcal{L}$ is recognized by some DFA, then it is \textbf{regular}.}

\defn{A \textbf{nondeterministic finite automaton (NFA), $M$, is a 5-tuple $(Q,\Sigma , \delta , q_0, F)$.}}
	\begin{enumerate}
		\item[] $Q$, the set of states.
		\item[] $\Sigma$, the alphabet
...
\defn{A string $s$ is \textbf{accepted} by and NFA $M$ if there is some path for $s$ from the start state to a final state.}
\defn{$M$ \textbf{recognizes} a language $\mathcal{L}$ consisting of all strings it accepts.}
