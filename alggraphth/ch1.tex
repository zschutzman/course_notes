\renewcommand{\exc}[1]{\subsection*{Exercise 1.#1}}
...
\exc{1} Let $X$ be a graph on $n$ vertices.  Show that $Aut(X)=Sym(n)$ if and only if $X$ is empty or complete.
...
\exc{2} Show that $Aut(X)=Aut(\overline{X})$.

Done in the notes.

\exc{3}  Show that automorphisms preserve distances between vertices.

Done in the notes.


\exc{4} Show that if $f:X\rightarrow Y$ is a homomorphism and $x_1,x_2$ are vertices in $X$, then $d_X(x_1,x_2)\geq d_Y(f(x_1),f(x_2))$.
...
\exc{5} Show that if $X$ is a graph and $Y$ a subgraph, and $f$ a homomorphism from $X$ to $Y$ such that the restriction of $f$ to $Y$ is a bijection that $Y$ is a retract.